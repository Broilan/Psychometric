...
exports.spearmanRankCorrelationCoefficient = spearmanRankCorrelationCoefficient;
/**
 ...
 */
function spearmanRankCorrelationCoefficient(x, y, rankX, rankY) {
    if ((!x || !y) && (!rankX || !rankY)) {
        throw new Error("Either provide datasets x and y, or provide rankX and rankY.");
    }
    // If raw data is provided, calculate the ranks
    const calculateRanks = (arr) => {
        return arr
            .map((val, i) => ({ val, index: i }))
            .sort((a, b) => a.val - b.val)
            .map((item, rank) => (Object.assign(Object.assign({}, item), { rank: rank + 1 })))
            .sort((a, b) => a.index - b.index)
            .map(item => item.rank);
    };
    const calculatedRankX = rankX !== null && rankX !== void 0 ? rankX : (x ? calculateRanks(x) : undefined);
    const calculatedRankY = rankY !== null && rankY !== void 0 ? rankY : (y ? calculateRanks(y) : undefined);
    ...
    const dSquaredSum = calculatedRankX.reduce((sum, rX, i) => sum + Math.pow((rX - calculatedRankY[i]), 2), 0);
